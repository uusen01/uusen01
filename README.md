# Hi, I'm Usen Usen 👋

## Senior Oracle Database Administrator

Senior Oracle DBA with 10+ years of experience supporting enterprise Oracle database environments across telecommunications, automotive, and USPS mail processing systems.

### Core Technologies

- Oracle 19c / 21c
- Oracle RAC
- Oracle Data Guard
- RMAN Backup & Recovery
- Oracle Multitenant (CDB/PDB)
- Performance Tuning
- Oracle GoldenGate
- OEM Cloud Control
- Linux & Windows Server
- SQL and PL/SQL
- AWS
- Disaster Recovery and High Availability

---

## Featured Repositories

### Oracle Health Checks
Scripts used to monitor database health, tablespaces, sessions, storage, and performance.

🔗 oracle-health-checks

### RMAN Scripts
Backup, recovery, restore and validation procedures.

🔗 oracle-rman-scripts

### Performance Tuning
SQL tuning and performance optimization scripts.

🔗 oracle-performance-tuning

### Oracle Patching Runbooks
Operational procedures and patching documentation.

🔗 oracle-patching-runbooks

### CDB/PDB Administration
Oracle multitenant administration scripts and procedures.

🔗 oracle-cdb-pdb-administration

### DBA Runbooks
Real-world operational procedures for Oracle database administration.

🔗 oracle-dba-runbooks

---

## Certifications

- OCI GoldenGate Workshop
- Oracle Cloud Infrastructure Foundations Associate
- Oracle Cloud Data Management Foundations Associate
- Oracle AI Foundations Associate
- Graduate Certificate in Information Security and Assurance

---

## Education

- M.S. Information Security & Assurance – Kennesaw State University
- M.S. Healthcare Management & Informatics – Kennesaw State University

---

## Connect With Me

- LinkedIn: www.linkedin.com/in/usen-usen
- Location: Atlanta, Georgia
